...
@section('content')
    <div class="dashboard-container mx-auto max-w-[1400px]">
        ...
        <div class="mb-8 flex items-center justify-between gap-4 rounded-xl surface p-5 shadow-sm max-md:flex-col max-md:items-start">
            <div class="property-selector">
                ...
                <div class="flex flex-wrap gap-3">
                    <select id="propertySelector" aria-label="Selecionar propriedade" class="min-w-[260px] rounded-lg border-2 border-subtle surface px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 transition-all hover:border-blue-500 focus:border-blue-500 focus:outline-none focus:ring-[3px] focus:ring-blue-500/10 max-md:min-w-full">
                    ...
                    @if($lavouras->isNotEmpty())
                        <select id="lavouraSelector" aria-label="Selecionar lavoura" class="min-w-[220px] rounded-lg border-2 border-subtle surface px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-300 transition-all hover:border-blue-500 focus:border-blue-500 focus:outline-none focus:ring-[3px] focus:ring-blue-500/10 max-md:min-w-full">
        ...
        @endphp
        <div class="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-[repeat(auto-fit,minmax(220px,1fr))] sm:gap-5">
        ...
        <div class="grid grid-cols-[2fr_1fr] gap-8 max-lg:grid-cols-1">
            ...
            <div class="flex flex-col gap-5">
                <div class="rounded-xl surface p-5 shadow-sm">
                    <div class="mb-5 flex flex-wrap items-center justify-between gap-2">
                        ...
                        <select aria-label="Período do gráfico de umidade" class="chart-period rounded-md border border-gray-300 dark:border-gray-600 surface px-3 py-1.5 text-sm">
                ...
                <div class="rounded-xl surface p-5 shadow-sm">
                    <div class="mb-5 flex flex-wrap items-center justify-between gap-2">
                        ...
                        <select aria-label="Período do gráfico de pH" class="chart-period rounded-md border border-gray-300 dark:border-gray-600 surface px-3 py-1.5 text-sm">
                            <option value="24h">24h</option>
                            <option value="7d" selected>7 dias</option>
                            <option value="30d">30 dias</option>
                        </select>
            ...
            <div class="flex flex-col gap-5">
                ...
                <div id="recomendacoes" class="overflow-hidden rounded-xl surface shadow-sm">
                    ...
                    <a href="{{ route('recomendacoes.index') }}" class="view-all-recomendacoes btn btn-outline mx-5 mb-5 mt-1">Ver Todas as Recomendações</a>
                </div>
                ...
                <div class="overflow-hidden rounded-xl surface shadow-sm">
                    ...
                    <div class="px-5">
                        @forelse($dadosDashboard['alertas'] as $alerta)
                            ...
                            @endphp
                            <a href="{{ route('alertas.marcarLido', $alerta->id_alerta) }}" class="alert-item flex flex-wrap items-center gap-2 border-b border-subtle py-4 last:border-b-0 hover-surface">
                                <div class="mr-1 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full {{ $corSeveridade['bg'] }} {{ $corSeveridade['text'] }}">
                                    <i class="fas fa-exclamation-circle"></i>
                                </div>
                                <div class="flex min-w-0 flex-1 flex-col gap-0.5">
                                    <span class="alert-title break-words text-sm font-medium text-heading">{{ $alerta->ds_mensagem }}</span>
                                    <span class="text-xs text-muted">{{ $alerta->dt_alerta->format('d/m/Y H:i') }}</span>
                                </div>
                                <div class="flex-shrink-0 rounded-xl {{ $corSeveridade['bg'] }} px-2 py-1 text-[11px] font-semibold uppercase {{ $corSeveridade['badgeText'] }}">
                ...
                <div class="overflow-hidden rounded-xl surface shadow-sm">
                    ...
                    <div class="grid grid-cols-[repeat(auto-fit,minmax(160px,1fr))] gap-3 p-5">
                        @forelse($dadosDashboard['sensoresStatus'] as $sensorStatus)
                            @php $online = $sensorStatus->ds_status?->value === 'ativo'; @endphp
                            <div class="flex items-center justify-between gap-2 rounded-lg border border-subtle p-3 transition-colors hover-surface">
                                <span class="truncate text-sm font-medium text-gray-700 dark:text-gray-300" title="{{ $sensorStatus->ds_nome }}">{{ $sensorStatus->ds_nome }}</span>
                                <span class="flex-shrink-0 whitespace-nowrap rounded-xl {{ $online ? 'bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-300' }} px-2 py-1 text-xs font-semibold uppercase">{{ $online ? 'Online' : 'Offline' }}</span>
                            </div>
                        @empty
                            <p class="text-sm text-muted [grid-column:1/-1]">
                                @if($selectedLavoura)
